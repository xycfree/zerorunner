...
const state = reactive({
  columns: [
    ...
    {key: 'intro', label: '简介', align: 'center', width: '', show: true
    },
    {key: 'url', label: '文件地址', align: 'center', width: '', show: true},
...
const getList = () => {
  tableRef.value.openLoading()
    userLeakfixApi().getList(state.listQuery)
      .then(res => {
...
const onOpenHistory = (row) => {
  console.log("row:", row)
  // router.push name:路由，菜单配置页面里的路由名称，不是ApiHistoryLeakfix.vue 里的 <script setup name="ApiHistoryLeakfix">
  // params 参数不会挂在地址上 2022-8-22更新已过期， 使用query会附在在地址上  https://locaohost:8893/xxx?name=name
  router.push({name: 'apiHistoryLeakfix', query: {name: row.name}})  //, query: {id: row.id}
