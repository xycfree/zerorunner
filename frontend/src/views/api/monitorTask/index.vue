<template>
  <div class="app-container">
    ...
    <EditMonitorTask ref="saveOrUpdateRef" @getList="getList"/>
...
<script setup name="MonitorTask">
import {h, onMounted, reactive, ref} from 'vue';
import {ElButton, ElDropdown, ElDropdownItem, ElDropdownMenu, ElMessage, ElMessageBox} from 'element-plus';
import EditMonitorTask from './EditMonitorTask.vue';
import {useMonitorTasksApi} from "/@/api/useAutoApi/monitorTasks";
...
const getList = () => {
  ...
  useMonitorTasksApi().getList(state.listQuery)
      .then(res => {
...
const taskSwitch = (row) => {
  ElMessageBox.confirm(`${row.enabled ? '停止' : '启动'}当前任务, 是否继续?`, '提示', {
    ...
    useMonitorTasksApi().taskSwitch({id: row.id})
        .then(() => {
...
const deleted = (row) => {
  ElMessageBox.confirm('是否删除该条数据, 是否继续?', '提示', {
    ...
    type: 'warning',
      ...
      .then(() => {
        useMonitorTasksApi().deleted({id: row.id})
            .then(() => {
...
const runOnceJob = (row) => {
  ElMessageBox.confirm("即将手动调度任务, 是否继续？", '提示', {
    ...
    useMonitorTasksApi().runOnceJob({id: row.id}).then(() => {
